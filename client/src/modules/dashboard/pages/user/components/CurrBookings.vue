...
function setBookings(){
  ...
  const now = new Date();
  currBookings.value = bookings.filter(booking => {
    const start = new Date(booking.startDate);
    const end = new Date(booking.endDate);
    return start <= now && now <= end;
  });

  currBookings.value.forEach(booking => {
...
<template>    
	...
	>
        ...
        <template #cancelBookingButton="slotProps">
			<Button class="booking__panel-buttons-cancel-booking"
				severity="danger"
            @click="$emit('bookings:action', { action: 'cancelBooking', bookingId: slotProps.bookingId })"
			>
				Cancel Booking
			</Button>
		</template>
