...
const queryString = computed(() => {
    ...
    if (state.filter.amenities && state.filter.amenities.length > 0)
        params.set("f_amenities", state.filter.amenities.join(","));
    ...
    <div class="layout">
        <div class="home">
            <div
                ...
                v-show="device.size >= 1024 || !isFilterClosed"
                :aria-hidden="device.size < 1024 && isFilterClosed ? 'true' : 'false'"
                :inert="device.size < 1024 && isFilterClosed ? '' : null"
            >
            ...
            <div class="home__right">
                <div class="search-filter-bar">
                    <Button v-if="device.size < 1024" @click="isFilterClosed = false">
                        Filter
                ...
                <div
                    ...
                    v-if="
                        state.searchInput ||
                        state.sort.hourlyRate ||
                        state.sort.capacity ||
                        state.sort.date ||
                        state.filter.hourlyRate != null ||
                        state.filter.capacity != null ||
                        (state.filter.amenities && state.filter.amenities.length)
                    "
                >
                    <Chip
                        v-if="state.searchInput"
                        :label="`Search: ${state.searchInput}`"
                        @remove="
                            () => {
                                state.searchInput = '';
                                syncUrlAndLoad();
                    ...
                    />
                    <Chip
                        v-if="state.sort.hourlyRate"
                        :label="`Rate: ${state.sort.hourlyRate}`"
                        @remove="
                            () => {
                                state.sort.hourlyRate = '';
                                syncUrlAndLoad();
                    ...
                    />
                    <Chip
                        v-if="state.sort.capacity"
                        :label="`Capacity: ${state.sort.capacity}`"
                        @remove="
                            () => {
                                state.sort.capacity = '';
                                syncUrlAndLoad();
                    ...
                    />
                    <Chip
                        v-if="state.sort.date"
                        :label="`Date: ${state.sort.date}`"
                        @remove="
                            () => {
                                state.sort.date = '';
                                syncUrlAndLoad();
                    ...
                    />
                    <Chip
                        v-if="state.filter.hourlyRate != null"
                        :label="`Max €/h: ${state.filter.hourlyRate}`"
                        @remove="
                            () => {
                                state.filter.hourlyRate = null;
                                syncUrlAndLoad();
                    ...
                    />
                    <Chip
                        v-if="state.filter.capacity != null"
                        :label="`Min cap.: ${state.filter.capacity}`"
                        @remove="
                            () => {
                                state.filter.capacity = null;
                                syncUrlAndLoad();
                    ...
                    />
                    <Chip
                        v-for="am in state.filter.amenities"
                        :key="am"
                        :label="am"
                        @remove="
                            () => {
                                state.filter.amenities = state.filter.amenities.filter((a) => a !== am);
                                syncUrlAndLoad();
                            }
                        "
                        removable
...
/* Hardening off-canvas behavior to prevent brief flashes when scrolling */
.filter-container {
...
/* Hide any sticky inner actions while closed */
.filter-container.filter-closed :deep(.filter__actions) {
...
@media (min-width: 1024px) {
    ...
    /* On desktop, ensure filter is static with no transform/transition to avoid flicker */
    .filter-container {
        width: 30%;
        position: static;
